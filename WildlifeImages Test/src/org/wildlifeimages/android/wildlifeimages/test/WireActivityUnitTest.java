 ...
 *
 */
public class WireActivityUnitTest extends ActivityUnitTestCase<IntroActivity>{

	Intent mockIntent = new Intent(Intent.ACTION_MAIN);

	public WireActivityUnitTest() {
	...
	public void testProcessActivityResult(){
		...
		int code_scan = mActivity.loadInt(R.integer.CODE_SCAN_ACTIVITY_REQUEST);
		...
		mActivity.onActivityResult(code_scan, Activity.RESULT_OK, intent);
		assertNull(getStartedActivityIntent());

		mActivity.onActivityResult(code_scan, Activity.RESULT_CANCELED, intent);
		assertNull(getStartedActivityIntent());


		code_scan = mActivity.loadInt(R.integer.CODE_SCAN_2_ACTIVITY_REQUEST);
		...
		for (String name : names){
			...
			intent.putExtra(extra_result_format, mActivity.loadString(R.string.intent_result_qr));
			mActivity.onActivityResult(code_scan+1, Activity.RESULT_OK, intent);
		...
		for (String name : names){
			intent.putExtra(extra_result, prefix + name);
			intent.putExtra(extra_result_format, mActivity.loadString(R.string.intent_result_qr));
			mActivity.onActivityResult(code_scan+1, Activity.RESULT_CANCELED, intent);
			Intent started = getStartedActivityIntent();
			assertNull(started);
		}
